...
## 📊 Forensic & Image Verification Gallery

Below are the actual visual stages and analytical distributions generated by the pipeline's core engine:

### 1. YOLOv8 Element Anchor Bounding Boxes
Localizes and extracts individual fields (`IT` badge, National `Logo`, unique alphanumeric `panNo` block, and verification `qr_code`).
![YOLOv8 Anchors](static/assets/yolov8_detections.jpg)

### 2. Document Preprocessing & Background Stripping
Isolates the document from any noisy background, transforming perspective geometry and normalising color parameters.
...
Uses Gaussian blurring, adaptive Otsu thresholding, and polygon approximation to map the exact boundary vector of the card.
![Contours](static/assets/contours.jpg)

### 4. Grayscale Pixel Intensity Distribution
Forensic histogram mapping of gray-level intensities, revealing print fidelity and structural contrast.
![Grayscale Distribution](static/assets/histogram.png)

### 5. HSV Multi-Channel Signature Mapping
Maps the Hue and Saturation profiles of the document to inspect for printing anomalies, color falsifications, and background copy-tampering.
